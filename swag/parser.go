...
func (parser *Parser) ParseAPI(searchDir, mainAPIFile string, parseDepth int) error {
	...
	if err != nil {
		fmt.Printf("warning: failed to get package name in dir: %s, error: %s\n", searchDir, err.Error())
	}
	...
	if parser.ParseDependency {
		...
		var pkgName string
		if pkgName, err = getPkgName(filepath.Dir(absMainAPIFilePath)); err != nil {
			return err
		}
...
func extractSecurityAttribute(context string, search, lines []string) (attrMap, scopes map[string]string, err error) {
...
func (parser *Parser) getTypeSchema(typeName string, file *ast.File, ref bool) (*spec.Schema, error) {
	...
	if ref && len(schema.Schema.Type) > 0 && schema.Schema.Type[0] == objectTypeValue {
		return parser.getRefTypeSchema(typeSpecDef, schema), nil
	}
...
func (parser *Parser) ParseDefinition(typeSpecDef *TypeSpecDef) (*Schema, error) {
	...
	if parser.isInStructStack(typeSpecDef) {
		return &Schema{
				...
				Schema:  PrimitiveSchema(objectTypeValue)},
...
func (parser *Parser) parseTypeExpr(file *ast.File, typeExpr ast.Expr, ref bool) (*spec.Schema, error) {
	...
	return PrimitiveSchema(objectTypeValue), nil
}
...
func (parser *Parser) parseStruct(file *ast.File, fields *ast.FieldList) (*spec.Schema, error) {
	...
	for _, field := range fields.List {
		...
		if err == ErrFuncTypeField { //nolint:gocritic // Don't want to rewrite as switch
			continue
		} else if err != nil {
			return nil, err
...
func (parser *Parser) parseStructField(file *ast.File, field *ast.Field) (schemaMap map[string]spec.Schema, tagsRequired []string, err error) {
	if field.Names == nil {
		if field.Tag != nil {
		...
		var typeName string
		if typeName, err = getFieldType(field.Type); err != nil {
			return nil, nil, err
		}
		var schema *spec.Schema
		if schema, err = parser.getTypeSchema(typeName, file, false); err != nil {
			return nil, nil, err
		}
		if len(schema.Type) > 0 && schema.Type[0] == objectTypeValue {
	...
	if structField.schemaType == stringTypeValue && types[0] != structField.schemaType {
	...
	eleSchema := schema
	if structField.schemaType == arrayTypeValue {
...
func (parser *Parser) parseFieldTag(field *ast.Field, types []string) (*structField, error) {
	...
	if len(types) > 1 && types[0] == arrayTypeValue {
	...
	if structField.schemaType == stringTypeValue || structField.arrayType == stringTypeValue {
	...
	if hasStringTag {
		...
		if defaultValue, ok := defaultValues[structField.schemaType]; ok {
			structField.schemaType = stringTypeValue
...
func (parser *Parser) GetSchemaTypePath(schema *spec.Schema, currentDepth int) []string {
	...
	if name := schema.Ref.String(); name != "" {
		if pos := strings.LastIndexByte(name, '/'); pos >= 0 {
			name = name[pos+1:]
			if schema, ok := parser.swagger.Definitions[name]; ok {
				return parser.GetSchemaTypePath(&schema, currentDepth)
			}
		}
	} else if len(schema.Type) > 0 {
		if schema.Type[0] == arrayTypeValue {
			currentDepth--
			s := []string{schema.Type[0]}
			return append(s, parser.GetSchemaTypePath(schema.Items.Schema, currentDepth)...)
		} else if schema.Type[0] == objectTypeValue {
			if schema.AdditionalProperties != nil && schema.AdditionalProperties.Schema != nil {
				// for map
				currentDepth--
				s := []string{schema.Type[0]}
				return append(s, parser.GetSchemaTypePath(schema.AdditionalProperties.Schema, currentDepth)...)
			}
		}
...
func (parser *Parser) replaceLastTag(element spec.Tag) {
	parser.swagger.Tags = append(parser.swagger.Tags[:len(parser.swagger.Tags)-1], element)
}
...
func (parser *Parser) checkOperationIDUniqueness() error {
	...
	for path := range parser.swagger.Paths.Paths {
		itm := parser.swagger.Paths.Paths[path]
		if itm.Get != nil {
