...
// Used to be: var paramPattern = regexp.MustCompile(`(\S+)[\s]+([\w]+)[\s]+([\S.]+)[\s]+([\w]+)[\s]+"([^"]+)"`)
// but the `[\S.]` intersects with itself, resolving to just `.`, which was likely unintended. Simplified as well.
...
func (operation *Operation) ParseParamComment(commentLine string, astFile *ast.File) error {
	...
	if strings.HasPrefix(refType, "[]") {
		objectType = arrayTypeValue
		refType = strings.TrimPrefix(refType, "[]")
		refType = TransToValidSchemeType(refType)
	} else if IsPrimitiveType(refType) ||
		paramType == "formData" && refType == "file" {
		objectType = primitiveTypeValue
	}

	requiredText := strings.ToLower(matches[4])
	required := requiredText == "true" || requiredText == requiredTextValue
	...
	case "path", "header":
		switch objectType {
		case arrayTypeValue, objectTypeValue:
	...
	case "query", "formData":
		...
		case objectTypeValue:
			...
			for _, name := range orderedNames {
				...
				if prop.Type[0] == arrayTypeValue && //nolint:gocritic // don't want to use switch here
					...
					fmt.Printf("skip field [%s] in %s is not supported type for %s\n", name, refType, paramType)
					continue
				}
...
func setNumberParam(name, schemaType, attr, commentLine string) (float64, error) {
	if schemaType != integerTypeValue && schemaType != numberTypeValue {
		return 0, fmt.Errorf("%s is attribute to set to a number. comment=%s got=%s", name, commentLine, schemaType)
	}
...
// This: var routerPattern = regexp.MustCompile(`^(/[\w\.\/\-{}\+:]*)[[:blank:]]+\[(\w+)]`)
// was simplified on recommendation from the linter
...
// This: var combinedPattern = regexp.MustCompile(`^([\w\-./\[\]]+)\{(.*)\}$`)
// was simplified on recommendation from the linter
...
func (operation *Operation) parseCombinedObjectSchema(refType string, astFile *ast.File) (*spec.Schema, error) {
	...
	for _, field := range fields {
		if matches := strings.SplitN(field, "=", 2); len(matches) == 2 {
			var s *spec.Schema
			if s, err = operation.parseObjectSchema(matches[1], astFile); err != nil {
				return nil, err
			}
			props[matches[0]] = *s
		}
...
// This: var emptyResponsePattern = regexp.MustCompile(`([\d]+)[\s]+"(.*)"`)
// was simplified on recommendation from the linter
